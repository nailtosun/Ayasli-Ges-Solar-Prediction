...
if( AnzahlWR==1 && WRInfo[0][5]>1 ) { // Multistring!
    ...
    if( WRInfo[0][5]==2 ) {
        if( status[0]!="Offline" ) {
            localPdc += aPdc[0] + aPdc[1]
            printRES_DayTextLeft2_2( aPdc[0], aPdc[1] )
        }
        else
        ...
        if( status[0]!="Offline" ) {
            localPdc += aPdc[0] + aPdc[1] + aPdc[2]
            printRES_DayTextLeft2_3( aPdc[0], aPdc[1], aPdc[2] )
        }
        else
